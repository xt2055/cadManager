...
test('预设只给基础名，变体后缀留给用户自己补', () => {
  for (const name of NEW_DRAWING_NAME_PRESETS) {
    // 杆坯1 / 杆坯2、缸头坯A / 缸头坯B 只是同名变体：点选基础名后接着打 1、A 就够。
    assert.doesNotMatch(name, /[0-9A-Za-z]$/, `${name} 是变体，不该单独占一条预设`)
  }
  for (const base of ['杆坯', '缸头坯']) {
    assert.ok(NEW_DRAWING_NAME_PRESETS.includes(base), `缺少基础名 ${base}`)
  }
})
