...
<template>
  <div class="page admin-page">
    ...
    <div class="card control-card"><div class="card-title"><DemoIcon name="folder-lock" :size="16" />隐藏 / 禁用对象<span class="hint">全部可逆 · 历史不丢</span></div><div class="table-pad"><table class="tbl"><thead><tr><th>编号</th><th>名称</th><th>操作</th><th>操作人</th><th>日期</th><th>恢复</th></tr></thead><tbody><tr v-for="(item, index) in demoStore.hiddenList" :key="item.no"><td class="num">{{ item.no }}</td><td class="operator">{{ item.name }}</td><td><span class="tag" :class="item.op === '隐藏' ? 'danger' : 'warn'">{{ item.op }}</span></td><td>{{ item.by }}</td><td class="num updated">{{ item.date }}</td><td><button class="btn sm primary" type="button" @click="demoStore.restoreHidden(index)"><DemoIcon name="rotate-ccw" :size="14" />恢复</button></td></tr><tr v-if="!demoStore.hiddenList.length"><td colspan="6"><div class="empty"><DemoIcon name="check-circle-2" :size="34" /><div class="t">没有隐藏或禁用的对象</div></div></td></tr></tbody></table></div></div>
