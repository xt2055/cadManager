...
async function restoreHidden(index: number) {
  try {
    await domainStore.restoreHidden(index)
  } catch (error) {
...
<template>
  <div class="page admin-page">
    ...
    <div class="card control-card"><div class="card-title"><DemoIcon name="folder-lock" :size="16" />隐藏 / 禁用对象<span class="hint">全部可逆 · 历史不丢</span></div><div class="table-pad"><table class="tbl"><thead><tr><th>编号</th><th>名称</th><th>操作</th><th>操作人</th><th>日期</th><th>恢复</th></tr></thead><tbody><tr v-for="(item, index) in domainStore.hiddenList" :key="item.no"><td class="num">{{ item.no }}</td><td class="operator">{{ item.name }}</td><td><span class="tag" :class="item.op === '隐藏' ? 'danger' : 'warn'">{{ item.op }}</span></td><td>{{ item.by }}</td><td class="num updated">{{ item.date }}</td><td><button class="btn sm primary" type="button" @click="restoreHidden(index)"><DemoIcon name="rotate-ccw" :size="14" />恢复</button></td></tr><tr v-if="!domainStore.hiddenList.length"><td colspan="6"><div class="empty"><DemoIcon name="check-circle-2" :size="34" /><div class="t">没有隐藏或禁用的对象</div></div></td></tr></tbody></table></div></div>
     <div class="card revert-card"><div class="card-title"><DemoIcon name="undo-2" :size="16" />版本回退</div><div class="card-pad revert-copy"><div>可将任意图纸的当前版本回退到历史版本。回退<b>不删除、不覆盖</b>任何版本——系统以目标版本内容生成新版本，全程留痕，可再次回退撤销。</div><div class="revert-action"><span>请先选择图纸</span><button class="btn sm primary" type="button" :disabled="!domainStore.drawings.length" @click="uiStore.openModal('revert', '版本回退')"><DemoIcon name="rotate-ccw" :size="14" />回退到历史版本</button></div></div></div>
  </div>
</template>
