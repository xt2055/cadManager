...
function selectedValue(kind: CandidateKind) {
  return kind === 'drawing' ? drawingNo.value : actorId.value
}
...
async function loadOptions(kind: CandidateKind, search = '') {
  ...
  try {
    const result = await listOperationLogOptions(true, kind, search)
...
async function loadLogs() {
  ...
  errorMessage.value = ''
  try {
    const next = await auditStore.loadAdmin({
      page: page.value,
...
<template>
  <div class="page admin-page log-admin-page">
    <div class="section-head">
      <div>
        <h3>操作日志</h3>
        <span class="lib-count">仅显示管理员执行的操作 · 显示 {{ visibleRange }} / {{ resultPage.total }} 条</span>
      </div>
    ...
    <section class="card filter-card">
      <div class="filter-grid">
        <div class="filter-item keyword-item">
          <label>关键字</label>
          <input v-model="query" class="inp" placeholder="搜索操作描述、图号、名称" @keyup.enter="search" />
        </div>
        ...
        <div class="filter-item">
          <label>操作类型</label>
          <select v-model="action" class="inp" @change="search">
        ...
        <div class="filter-item">
          <label>对象类型</label>
          <select v-model="targetType" class="inp" @change="search">
            <option value="">全部对象</option>
            <option v-for="[code, label] in targetOptions" :key="code" :value="code">{{ label }}</option>
          </select>
        </div>
        <div class="filter-item">
          <label>结果</label>
          <select v-model="result" class="inp" @change="search">
        ...
        <div class="filter-item">
          <label>开始日期</label>
          <input v-model="from" class="inp" type="date" @change="search" />
        </div>
        <div class="filter-item">
          <label>结束日期</label>
          <input v-model="to" class="inp" type="date" @change="search" />
        </div>
      </div>
      <div class="filter-actions">
        <button class="btn primary sm" type="button" @click="search">
        ...
        <table class="tbl log-table">
          <thead>
            <tr>
              ...
              <th>操作描述</th>
              <th>结果</th>
              <th style="width: 70px; text-align: center;">详情</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="item in rows" :key="item.id">
              ...
              <td>
                ...
                <small v-if="item.userAccount" class="sub-text">{{ item.userAccount }}</small>
              </td>
              <td>
                <span class="activity-tag" :class="colors[item.act]">
                  <DemoIcon :name="feedIcons[item.act]" :size="13" />
              ...
              <td style="text-align: center;">
                <button class="icon-btn" type="button" title="查看详情" @click="openDetail(item)">
                  <DemoIcon name="list" :size="15" />
                </button>
              </td>
            </tr>
          </tbody>
    ...
    <div v-if="selectedLog" class="detail-overlay" role="presentation" @click.self="selectedLog = null">
      <aside class="detail-drawer" role="dialog" aria-modal="true" aria-label="操作日志详情">
        <header class="drawer-head">
          <div>
            <span class="eyebrow">管理员审计详情</span>
            <h2>{{ selectedLog.txt }}</h2>
          </div>
          <button class="icon-btn" type="button" aria-label="关闭详情" @click="selectedLog = null">
            <DemoIcon name="x" :size="17" />
          </button>
        </header>
        <div class="drawer-body">
          <div class="detail-grid">
            <div><span>操作人</span><strong>{{ selectedLog.user }}</strong></div>
            <div><span>时间</span><strong class="mono">{{ selectedLog.time }}</strong></div>
            <div><span>操作类型</span><strong>{{ ACTIVITY_LABELS[selectedLog.act] }}</strong></div>
            <div><span>对象类型</span><strong>{{ targetLabel(selectedLog.targetType) }}</strong></div>
            <div><span>图号</span><strong class="mono">{{ selectedLog.drawingNo || '—' }}</strong></div>
            <div><span>结果</span><strong :class="selectedLog.result === 'success' ? 'success-text' : 'danger-text'">{{ selectedLog.result === 'success' ? '成功' : '失败' }}</strong></div>
          </div>
          ...
          <div class="detail-section">
            <span class="detail-label">附加详情</span>
...
.activity-tag.info { color: var(--accent); background: var(--accent-soft); }
.activity-tag.ok { color: var(--success, #35b87f); background: color-mix(in srgb, var(--success, #35b87f) 12%, transparent); }
.activity-tag.warn { color: var(--warning, #e9a33a); background: color-mix(in srgb, var(--warning, #e9a33a) 12%, transparent); }
.activity-tag.plain { color: var(--text-2); background: var(--hover); }
.activity-tag.danger { color: var(--danger); background: color-mix(in srgb, var(--danger) 12%, transparent); }
.summary-cell {
  max-width: 260px;
  color: var(--text-2);
}
...
.success-text {
  color: var(--success, #35b87f) !important;
}
.danger-text {
