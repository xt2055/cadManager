...
export interface BorrowCommandGateway {
  borrow(drawingId: string, input: BorrowPartInput, idempotencyKey?: string): Promise<DrawingBorrowResult>
}
...
export class BorrowCommandService {
  ...
  borrow(drawingId: string, sourcePartId: string, borrowReason?: string, idempotencyKey?: string): Promise<DrawingBorrowResult> {
