...
在现有系统中增加“零件索引”导航。用户能够搜索项目中已有的零件 CAD 附件，查看标题栏信息、按需修正异常识别结果、查看关联项目，并打开对应附件进行 CAD 预览。完整自动识别结果直接可用，不要求逐条确认。
...
6. 选择标题栏布局、选择候选值、人工编辑和保存修订。
7. 已有快照批量补建索引，作为管理员高级维护工具。
8. 用户在浏览器中对所选附件强制重新读取，或按筛选范围跨页串行补齐待提取、失败附件。
...
提取完成后，图号和零件名称完整的结果可以直接用于检索，不要求逐条人工确认。自动识别仍不写确认人或确认时间；用户发现错误时按需保存人工修订。

### 7.2 页面处理状态

API `status` 按以下优先级从上到下计算：

1. 历史 `confirmed_at` 非空，且 `confirmed_snapshot_revision != source_snapshot_revision`：`recheck`，显示“换版待检查”。
2. 历史 `confirmed_at` 非空，且两个快照修订号相等：`confirmed`，显示“已人工确认”，仅用于兼容已有记录。
3. `extraction_status='failed'`：`failed`，显示“提取失败”。
4. `extraction_status='pending'`：`pending`，显示“待提取”。
5. 生效标题栏图号或零件名称为空：`needs_confirmation`，显示“需检查”。多个有效布局无法自动选择时生效字段为空，也进入此状态。
6. 有 `manual_fields`：`edited`，显示“已人工修订”。
7. 其他提取完整的自动结果：`recognized`，显示“已识别”。

只允许上述七种 API 状态。`confirmed` 和 `recheck` 是历史确认数据的兼容状态；新流程不要求人工确认。不要再创造 `stale`、`ready`、`done` 等同义状态。
...
| status | 空 | pending / failed / needs_confirmation / recognized / edited / confirmed / recheck |
...
- 快照失败时 `status` 为 failed；快照有效但索引尚未建立时因生效图号和名称为空，`status` 为 needs_confirmation。
- 显示“索引尚未建立”，有写权限者可点击同步已保存标题栏。
- 不在 GET 请求里偷偷写数据库。
...
- 处理状态选择。
- 重置筛选。
- 全选或取消选择当前页可写条目；“读取所选图纸”允许强制重新读取已识别文件。
- “生成待完善索引”按当前筛选跨页收集待提取、失败文件，再串行读取或利用有效快照补齐索引。
- 显示固定总数、当前文件、停止和失败项重试。
- admin 在折叠的“高级维护”中显示“修复已有索引（不读取图纸）”。
...
完整自动识别结果显示“已识别”，直接用于检索。标题栏图号或零件名称缺失时显示“需检查”及具体提示；多有效空间时显示空选择和明确提示，不默认取第一个。
...
- 布局切换先只影响本地表单；用户点击保存修改后才持久化。
- `selectedSpaceId=null` 选项显示“人工填写，不绑定布局”。
- 展示“保存修改”和“恢复识别值”按钮，不把人工确认作为必经步骤。
- “恢复识别值”执行 reset，点击前确认会清除当前版本全部人工修订和历史确认记录。
- 保存期间禁止重复提交及重新提取。
