...
func StartPatentReminders(ctx context.Context, pool *pgxpool.Pool) {
    ...
    SELECT p.id,p.deadline_source,x.kind,x.deadline,
   ...
   SELECT id,NULL,'reminder',jsonb_build_object('kind',kind,'deadline',deadline,'stage',stage,'days',days,'source',deadline_source)
